...
public class ConfigActivity extends BaseActivity
{
	...
	protected void onCreate(Bundle savedInstanceState)
	{
		...
		spawnprotect_toggle.setOnCheckedChangeListener(new OnCheckedChangeListener()
			{
				@Override
				public void onCheckedChanged(CompoundButton sender, boolean checked)
				{
		...
		whitelist_edit.setOnClickListener(new OnClickListener() 
			{
				...
				public void onClick(View v)
				{
					startActivity(new Intent(ConfigActivity.this, WhitelistActivity.class));
				}
			});
		...
		if (values.containsKey("spawn-protection") && !values.get("spawn-protection").equals("-1"))
		{
			try
			{
				spawnprotect.setText(Integer.parseInt(values.get("spawn-protection")) + ""); // no, that isn't a
				// nonsense
		...
		setValue(R.id.config_desc, "description", "使用PocketMine-MP开服");
		setValue(R.id.config_motd, "motd", "欢迎 @player 来到此服务器！");
		...
		setValue(R.id.config_rcon_port,"rcon.port","25555");
	...
	public void save()
	{
		...
		putValueString(R.id.config_rcon_port,"rcon.port");
		putValueBool(R.id.config_autosave, "auto-save");

		try
		{
			PrintWriter writer = new PrintWriter(ServerUtils.getDataDirectory() + "/server.properties");
	...
	private void readFile()
	{
		...
		try
		{
			BufferedReader reader = new BufferedReader(new FileReader(ServerUtils.getDataDirectory() + "/server.properties"));
			try
			{
				String line;
